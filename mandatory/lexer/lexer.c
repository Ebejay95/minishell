...
/*   Updated: 2024/08/07 14:05:00 by jeberle          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void prompt_to_token(t_minishell *m)
{
    ...
    while (*ptr)
    {
        ...
        {
            current_token[current_pos] = *ptr;
            expmap[current_pos] = quote_level == 1 ? '1' : '0';
            current_pos++;
            escape_next = 0;
            ptr++;
            continue;
        ...
        if (*ptr == '\'' || *ptr == '"')
        {
            ...
            else if ((quote_level == 1 && *ptr == '\'') || (quote_level == 2 && *ptr == '"'))
            {
                quote_level = 0;
            }
            else
            {
                current_token[current_pos] = *ptr;
                expmap[current_pos] = quote_level == 1 ? '1' : '2';
                current_pos++;
            }
            ptr++;
            continue;
        ...
        if ((*ptr == '>' || *ptr == '<') && quote_level == 0)
        {
            ...
            if (*(ptr + 1) == *ptr)
            {
                token = create_token(*ptr == '>' ? ">>" : "<<", "00");
                ptr++;
            }
            else
                token = create_token(*ptr == '>' ? ">" : "<", "0");
            update_tok_type(token, REDIRECTION);
            add_token_to_list(&m->tok_lst, token);
            ptr++;
            continue;
        ...
        current_token[current_pos] = *ptr;
        expmap[current_pos] = quote_level == 1 ? '1' : (quote_level == 2 ? '2' : '0');
        current_pos++;
        if (current_pos >= current_token_size)
        {
            current_token_size *= 2;
