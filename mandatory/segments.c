...
/*   Updated: 2024/07/16 22:27:29 by jeberle          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_segment	**lex(char *str, t_toktype lookfor)
{
	...
	while (str[i])
	{
		if (lookfor == REDIRECTION)
			new_pos = is_redirection_pattern(str, i);
		else if (lookfor == PIPE)
			new_pos = is_pipe_pattern(str, i);
		else if (lookfor == GETEXSTATE)
			new_pos = is_ges_pattern(str, i);
		else if (lookfor == VARIABLE)
			new_pos = is_var_pattern(str, i);
		else
			new_pos = 0;
		if (new_pos)
		{
			if (i > last_pos)
			{
				...
				segments[count++] = create_segment(str, last_pos, i, ARGUMENT);
			}
			segments = realloc(segments, sizeof(t_segment *) * (count + 1));
			segments[count++] = create_segment(str, i, new_pos, lookfor);
			last_pos = new_pos;
			i = new_pos;
		}
		else
